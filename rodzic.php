...
<head>
    ...
    <style>
        ...
        .btn-logout { 
            ...
            border: 1px solid #d32f2f; padding: 8px 15px; border-radius: 5px; 
            transition: 0.3s;
        }
        ...
        .tile {
            ...
            position: relative;
            overflow: hidden;
            border-top: 5px solid #2196F3; /* Niebieski pasek na górze */
        }

        /* Efekt najechania myszką */
        .tile:hover {
            transform: translateY(-10px); /* Uniesienie */
            box-shadow: 0 15px 35px rgba(33, 150, 243, 0.2); /* Większy, niebieskawy cień */
        }
...
<body>
    ...
    <div class="container">
        ...
        <h1>Twoje Dzieci</h1>
        <p class="subtitle">Wybierz ucznia, aby sprawdzić jego postępy w nauce.</p>

        <div class="tiles-grid">
            <?php if (count($dzieci) > 0): ?>
                
                <?php foreach ($dzieci as $d): ?>
                    <a href="oceny.php?id_uzytkownika_dziecka=<?php echo $d['id_uzytkownika']; ?>" class="tile">
                        <span class="tile-icon">🎓</span>
                        <span class="tile-name"><?php echo $d['imie']; ?></span>
                        <span class="tile-class">Klasa <?php echo $d['klasa'] ?? 'Brak'; ?></span>
                        
                        <span class="tile-btn">Zobacz oceny →</span>
                    </a>
                <?php endforeach; ?>
